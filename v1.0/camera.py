...
try:
    ...
    while True:
        ...
        # mask = normalizationFunc(mask)
        # storingPxs(mask, "dataRight.pickle",
        #            "labelsRight.pickle", [0, 1, 0, 0])
        # i += 1
        # print(i)

        preprocessing.normalize(mask)
        test = []
        for j in mask.tolist():
            # print(j)
            for k in j:
                test.append(k)
        # test.append(lisx)

        pred = tmodel.predict([test])
        print(pred)
        # if pred[0][0] == 1:
        #     cv2.putText(img, 'forward', (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
        #                 1, (0, 255, 0), 2)
